	...
	Layer.prototype = {
		...
		drag : function(){
			...
			util.addHandler(layer,"mousedown",function(event){
				...
				var diffX=event.clientX-layer.offsetLeft;
				var diffY=event.clientY-layer.offsetTop;
				
				var srcElement = event.target || event.srcElement;
				if(srcElement.tagName=="H2"){  //鼠标点下的目标元素是h2时才可以拖动
				...
				function move(event){
					...
					_this.style.top=top+"px"; //根据左上角移动
					
				}
				 //-----------mouseup事件执行的函数-----------//
				function up(){
					//-------IE8存在bug: 拖拽完成后无法释放鼠标
					//alert("")
